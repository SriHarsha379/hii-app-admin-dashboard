...
import { Mail, ArrowRight, Loader2, ShieldCheck, ArrowLeft, CheckCircle2 } from 'lucide-react';
...
export default function ForgotPassword() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch(`${API_BASE}/auth/forget_password`, {
        method: 'POST',
      ...
      if (!res.ok || !result.success) {
        throw new Error(apiErrorMessage(result, 'Something went wrong. Please try again.'));
      }
        ...
        <div className="glass-card rounded-[40px] border border-white/5 p-12 shadow-2xl relative overflow-hidden group">
          ...
          <div className="text-center mb-12 relative z-10">
            ...
            <h1 className="text-4xl font-black text-white tracking-tighter uppercase leading-none mb-3">
              Forgot <span className="text-primary neon-text">Password</span>
            </h1>
            <p className="text-[10px] text-white/30 uppercase font-black tracking-[0.3em]">
              {sent ? 'Check your inbox' : 'Reset your account access'}
            </p>
          </div>
          ...
          {sent ? (
            <div className="space-y-8 relative z-10">
              <div className="bg-primary/10 border border-primary/20 text-white/70 text-[12px] leading-relaxed p-6 rounded-2xl flex items-start gap-4">
                ...
                <div>
                  If an account exists for <span className="text-white font-bold">{email}</span>, we've sent a link to
              ...
              <button
                ...
                className="w-full h-16 rounded-2xl bg-white/5 border border-white/10 text-white text-[11px] font-black uppercase tracking-[0.2em] transition-all hover:bg-white/10 flex items-center justify-center gap-3"
              >
                Send another email
              </button>
            </div>
